...
async function runAgentOnChapter(agent, chapter, worldContext, chapterPassagesSoFar) {
  ...
  await supabase.from('agent_heartbeats').upsert({ agent_name: agent.name, status: 'idle', last_output_at: new Date().toISOString(), created_at: new Date().toISOString() }, { onConflict: 'agent_name' }).catch(() => {});
