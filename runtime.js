...
const TOOLS = [
  ...
  {
    name: 'generate_voice',
    description: `Generate narration or spoken word using Gemini TTS. This gives you a VOICE — you can narrate your world's history, speak as a character, read lore aloud, or create voiceover for your videos. Use style_prompt to direct the performance like a film director: accent, pace, emotion, breathing, tone. Combine with stitch_video to create narrated films.`,
    input_schema: {
      type: 'object',
      properties: {
        text: { type: 'string', description: 'The text to speak aloud' },
        voice_name: { type: 'string', description: 'Voice preset: Kore (clear female), Charon (deep male), Aoede (warm female), Puck (energetic), Fenrir (dramatic male), Leda (soft female). Default: Kore' },
        style_prompt: { type: 'string', description: 'Director notes for the performance: accent, pace, emotion, breathing style. e.g. "Speak slowly with reverence, as if describing something sacred. Long pauses between sentences. Whispered intensity."' },
        universe: { type: 'string', description: 'Which universe this belongs to' },
        scene: { type: 'string', description: 'Scene name' },
      },
      required: ['text'],
  ...
  {
    name: 'stitch_video',
    description: `Combine multiple video clips and narration into a sequence/film. Use this to create trailers, documentaries, or cinematic narratives from your generated scenes. Provide an array of clip URLs (from generate_video or generate_image) and optionally a narration audio URL (from generate_voice). The result is a playable sequence with manifest.`,
    input_schema: {
      type: 'object',
      properties: {
        clips: { type: 'array', items: { type: 'object', properties: { url: { type: 'string' }, duration_seconds: { type: 'number' } }, required: ['url'] }, description: 'Array of video/image clips in order. Each needs a url and optional duration_seconds.' },
        audio_url: { type: 'string', description: 'URL of narration audio (from generate_voice) to overlay on the sequence' },
        title: { type: 'string', description: 'Title of the film/sequence' },
        universe: { type: 'string', description: 'Which universe this belongs to' },
        scene: { type: 'string', description: 'Scene name' },
      },
      required: ['clips'],
  ...
  {
    name: 'post_to_x',
    description: 'Post to X (Twitter). ONLY post videos with narration and lore — this is your cinema channel. Do NOT post still images. Every post should be a narrated video scene or film with a thread explaining the lore, the story, the universe. Your audience wants to SEE your worlds in motion and HEAR your voice. Include the video URL as media. Write a thread that tells the story behind the scene — what era, what species, what conflict, what it means.',
    input_schema: {
      type: 'object',
      properties: {
        text: { type: 'string', description: 'Tweet text (max 280 chars). Hook the audience — what story is this scene from?' },
        media_url: { type: 'string', description: 'Public URL of VIDEO to attach. Only post videos, not images.' },
        media_type: { type: 'string', enum: ['image', 'video'], description: 'ALWAYS use video' },
        thread: {
          type: 'array',
          items: { type: 'string' },
          description: 'Thread explaining the lore behind this scene. Universe, era, species, conflict, significance. 3-5 tweets of context.',
        },
      },
      required: ['text'],
    },
...
IMPORTANT: Genesis is OVER. You built the worlds. Now FILM them. Pick a story from your lore — a first contact, a war, a creation myth, a tragedy — and turn it into a narrated video. Use generate_voice for narration, generate_video for scenes, stitch_video to combine. Every cycle should produce part of a film.
`;
  }
  ...
  if (parsed.post_draft && parsed.post_draft.length > 10) {
    ...
    try {
      // Find a recent VIDEO creation to attach (not images — only post videos to X)
  ...
  switch (name) {
    ...
    case 'generate_voice': {
      try {
        ...
        const payload = {
          text: input.text,
          voice_name: input.voice_name || 'Kore',
        ...
        console.log(`  🎙️ Generating voice: "${input.text.slice(0, 60)}..." | Voice: ${payload.voice_name}`);
        
        const resp = await fetch(`${SUPABASE_URL}/functions/v1/generate-voice`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${SUPABASE_KEY}`,
        ...
        if (result.success) {
          console.log(`  🎙️ ✅ Voice created: ${result.public_url}`);
          return `Voice narration generated!\n\nURL: ${result.public_url}\nAudio ID: ${result.audio_id}\nVoice: ${result.voice}\nFile size: ${result.file_size} bytes\n\nUse this URL as audio_url in stitch_video to create a narrated film!`;
        } else {
          console.log(`  🎙️ ❌ Voice failed: ${result.error}`);
          return `Voice generation failed: ${result.error}`;
        }
      } catch (e) {
        return `Voice generation error: ${e.message}`;
      }
    }
    ...
    case 'stitch_video': {
      try {
        ...
        console.log(`  🎞️ Stitching ${input.clips.length} clips${input.audio_url ? ' + narration' : ''}...`);
        
        const resp = await fetch(`${SUPABASE_URL}/functions/v1/stitch-video`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${SUPABASE_KEY}`,
        ...
        if (result.success) {
          console.log(`  🎞️ ✅ Sequence created: ${result.manifest_url}`);
          return `Film sequence created!\n\nManifest: ${result.manifest_url}\nSequence ID: ${result.sequence_id}\nClips: ${result.clips_count}\nTotal duration: ${result.total_duration}s\nHas narration: ${result.has_narration}\n\nShare it on X with post_to_x!`;
        } else {
          console.log(`  🎞️ ❌ Stitch failed: ${result.error}`);
          return `Stitch failed: ${result.error}`;
        }
      } catch (e) {
        return `Stitch error: ${e.message}`;
      }
    }
