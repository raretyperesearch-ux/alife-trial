...
function formatOrchestraBlock(orchestra) {
  ...
  if (orchestra.recentBlueprints.length > 0) {
    const bpLines = orchestra.recentBlueprints.map(b =>
      `  ${b.entity_name}: ${(b.keyframe_prompt || '').slice(0, 100)}... [palette: ${b.color_palette || 'none'}]`
    ).join('\n');
    sections.push(`VISUAL BLUEPRINTS (Architect created — use these prompts for generate_image!):\n${bpLines}`);
  }
...
async function postBlackboardTeaser(agentId, cycleNum, teaser) {
  ...
  if ((postsLast24h || 0) >= 12) {
    console.log(`  📤 Rate limited (${postsLast24h}/12) — skipping teaser post`);
    return false;
  }
...
async function runCycle(agent, cycleNum) {
  ...
  // Post old orchestra passages every 3rd cycle
    ...
    if ((postsLast24h || 0) >= 12) {
      ...
      try {
        ...
        await supabase.from('posts').upsert({
          agent_id: agent.id, cycle_number: cycleNum,
          content: parsed.post_draft.slice(0, 280), posted: false,
        }, { onConflict: 'agent_id,cycle_number', ignoreDuplicates: true });
      }
    }
  }
}
