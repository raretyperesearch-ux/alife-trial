...
async function writeAngelOutput(task, angel, output) {
  ...
  switch (type) {
    ...
    case 'update_entity': {
      ...
      if (type === 'update_entity' && output.id) {
        const { data, error } = await supabase
          .from('entity_profiles')
    ...
    case 'create_timeline_event': {
      const { data } = await supabase
        ...
        .insert({
          ...
          significance: output.significance || 5,
          created_by_agent: angel.id
        })
        .select()
        .single();
    ...
    case 'update_blueprint': {
      const { data } = await supabase
        ...
        .insert({
          ...
          blueprint_type: output.blueprint_type || 'character',
          universe: output.universe,
          scene_description: output.scene_description,
          keyframe_prompt: output.keyframe_prompt,
          color_palette: output.color_palette || {},
          lighting_notes: output.lighting_notes,
          materials_notes: output.materials_notes,
    ...
    case 'create_disruption': {
      const { data } = await supabase
        ...
        .insert({
          title: output.title,
          description: output.description,
          side_a: output.side_a,
          side_b: output.side_b,
          stakes: output.stakes,
          universe: output.universe,
          status: output.status || 'brewing',
          created_by_agent: angel.id
        })
        .select()
        .single();
    ...
    case 'challenge_work': {
      const { data } = await supabase
        .from('challenges')
        .insert({
          challenger_agent: angel.id,
          target_table: output.target_table,
    ...
    case 'create_prophecy_card': {
      const teaserType = type === 'create_preview' ? 'preview'
        : type === 'create_prophecy_card' ? 'prophecy_card'
    ...
    case 'write_narration': {
      // Narration comes as array of shots
      const shots = Array.isArray(output) ? output : output.shots || [output];
      const records = shots.map(s => ({
        episode_pitch_id: s.episode_pitch_id,
        shot_number: s.shot_number,
        narration_text: s.narration_text,
        voice_character: s.voice_character,
        emotional_tone: s.emotional_tone,
        is_silence: s.is_silence || false,
