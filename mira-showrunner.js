    ...
    switch (normalizedType) {
      ...
      case 'update_blueprint': {
        const { data, error } = await supabase.from('visual_blueprints').insert({
          entity_id: output.entity_id, entity_name: output.entity_name,
      ...
      case 'create_disruption': {
        if ((normalizedType === 'escalate_conflict' || normalizedType === 'resolve_conflict') && output.conflict_id) {
          ...
          if (error) console.error(`   ⚠ DB error (conflict update):`, error.message);
          return { table: 'conflicts', id: data?.id || output.conflict_id };
        }
        const { data, error } = await supabase.from('conflicts').insert({
          title: output.title, description: output.description,
      ...
      case 'write_dialogue': {
        const { data, error } = await supabase.from('monologues').insert({
          entity_name: output.entity_name, entity_id: output.entity_id, universe: output.universe,
          content: output.content, context: output.context, emotional_tone: output.emotional_tone,
          created_by_agent: angel.id
        }).select().single();
...
async function smartRouteOutput(angel, output) {
  if (output.content && output.entity_name && output.emotional_tone) {
    const { data, error } = await supabase.from('monologues').insert({
      entity_name: output.entity_name, content: output.content,
      emotional_tone: output.emotional_tone, universe: output.universe, created_by_agent: angel.id
    }).select().single();
...
async function runCycle() {
  ...
  // 2. Skip if too many pending tasks
  if (blackboard.pendingTasks.length >= 5) {
    console.log('⏸️  Too many pending tasks, waiting for angels to finish...');
    return;
  }
