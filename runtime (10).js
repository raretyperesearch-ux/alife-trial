...
`;
  ...
  if (parsed.forge) {
    ...
    // Build descriptive memory based on action type
    const forgeLabel = parsed.forge.target_skill || parsed.forge.skill_id || parsed.forge.name || 'unknown';
    const forgeAction = parsed.forge.action || 'unknown';
    let forgeMemory;
    
    if (forgeAction === 'invoke_forged') {
      forgeMemory = forgeResult.success 
        ? `Invoked tool: ${forgeLabel}. Result: ${JSON.stringify(forgeResult.result || forgeResult).slice(0, 300)}`
        : `Failed to invoke tool: ${forgeLabel}. Error: ${forgeResult.reason || forgeResult.error || 'unknown'}`;
    } else if (forgeAction === 'deploy_edge_function') {
      forgeMemory = forgeResult.success
        ? `Built and deployed tool: ${forgeLabel}. ${parsed.forge.description || ''} Callable via invoke_forged.`
        : `Failed to deploy tool: ${forgeLabel}. Error: ${forgeResult.reason || 'unknown'}`;
    } else if (forgeAction === 'list_forged_skills') {
      const skillList = forgeResult.functions?.map(f => f.skill_id).join(', ') || 'none';
      forgeMemory = `Listed forged skills: ${skillList}`;
    } else {
      forgeMemory = `Forge ${forgeAction} on ${forgeLabel}: ${forgeResult.success ? 'success' : 'failed'}`;
    }

    await storeMemory(agent.id, {
      key: `forge_${forgeAction}_${forgeLabel}_cycle_${cycleNum}`,
      value: forgeMemory,
      category: 'forge',
      importance: forgeResult.success ? (forgeAction === 'deploy_edge_function' ? 9 : 7) : 6,
      content: forgeMemory,
    });
  }
